...
public class Arith{ //默认除法运算精度
     ...
     * 浮点数是否相等
     */
    public static boolean compareEqualNum(double v1,double v2) {
        BigDecimal b1 = new BigDecimal(Double.toString(v1));
        BigDecimal b2 = new BigDecimal(Double.toString(v2));
        return b1.compareTo(b2) == 0;
    }
    ...
    public static double getBigDecimalNum(double num,int scale) {
    ...
    public static double arithBasisPercent(double priceOne,double priceTwo) {
        return Math.abs(div(sub(priceOne,priceTwo),priceOne));
    }
    /**
     * 手续费
    ...
    public static double arithFee(double denomination,int volume,double feePercent) {
        return mul(mul(denomination,volume),div(feePercent,100));
    }

    /**
     * 获取一个数的百分之几
     */
    public static double getPercentNum(double num, double decimalsNum) {
        String newNum = String.format("%.2f", num * decimalsNum);
     ...
     * 计算数额
     */
    public static int getCountPrice(JSONArray arr,int flag) {
        int size = Math.min(arr.size(), flag);
        int num = 0;
        for (int i = 0; i < size; i++) {
            JSONArray depth = arr.getJSONArray(i);
//            num += (depth.getIntValue(0) / depth.getIntValue(1));
            num += (100 * depth.getIntValue(1));
        }
        return num;
    }
}